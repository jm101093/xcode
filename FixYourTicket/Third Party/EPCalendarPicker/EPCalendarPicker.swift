...
open class EPCalendarPicker: UICollectionViewController {
    ...
    func inititlizeBarButtons(){
        ...
        let cancelButton = UIBarButtonItem(image: UIImage(named:"cross_icon"), style: .done, target: self, action: #selector(EPCalendarPicker.onTouchCancelButton))
        self.navigationItem.leftBarButtonItem = cancelButton
        
       
          //  let reset = UIBarButtonItem(title: "Reset", style: .done, target: self, action: #selector(EPCalendarPicker.onTouchResetButton))
      ...
      //      let todayButton = UIBarButtonItem(image: #imageLiteral(resourceName: "Reset"), style: .done, target: self, action: #selector(EPCalendarPicker.onTouchResetButton))
        //    arrayBarButtons.append(todayButton)
          //  todayButton.tintColor = UIColor.white
       // }
    ...
    override open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row >= prefixDays {
            ...
            if arrSelectedDates.filter({ $0.isDateSameDay(currentDate)
                ...
                if (currentDate.isGreaterThanDate(dateToCompare: nextMonthFirstDay)) {
                ...
                if startDate != nil {
                    if Calendar.current.startOfDay(for: cell.currentDate as Date).isLessThanDate(dateToCompare: Calendar.current.startOfDay(for: startDate!)) {
                        cell.isCellSelectable = false
                        cell.lblDay.textColor = self.dayDisabledTintColor
                    }
                }
    ...
    internal func onTouchResetButton(){
