...
extension UIImageView {
                                       ...
                                       failure:((_ request:URLRequest?, _ response:URLResponse?, _ error:NSError) -> Void)?)
    {
        ...
        if let cachedImage = UIImageView.sharedImageCache().cachedImageForRequest(request: request as NSURLRequest) {
            if success != nil {
                ...
                DispatchQueue.main.async(execute: { () -> Void in
                    if request.url! == self.af_requestImageOperation.request?.url {
                        ...
                        if image != nil {
                            ...
                            UIImageView.sharedImageCache().cacheImage(image: image!, forRequest: request as NSURLRequest)
                        }
...
class AFImageCache: NSCache<AnyObject, AnyObject>, AFImageCacheProtocol {
    func cachedImageForRequest(request: NSURLRequest) -> UIImage? {
        ...
        return self.object(forKey: AFImageCacheKeyFromURLRequest(request as URLRequest) as AnyObject) as? UIImage
    }
    
    func cacheImage(image: UIImage, forRequest request: NSURLRequest) {
        self.setObject(image, forKey: AFImageCacheKeyFromURLRequest(request as URLRequest) as AnyObject)
    }
}
